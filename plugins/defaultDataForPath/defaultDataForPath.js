var jsonData = [
    {
        ...
        paths: ["C:\\Users\\UserName\\Desktop\\NOTPORN\\Brazzers", "D:\\SecretStorage\\Porn\\Brazzers"],
        studio: "Brazzers",
...
function main() {
    ...
    // create tags
    var defaultTags = []
    for (let p = 0; p < defaultData.length; p++) {
        let tags = defaultData[p].tags
        if (tags) {
    ...
    var defaultStudio
    for (let p = 0; p < defaultData.length; p++) {
        let studio = defaultData[p].studio
        if (studio) {
    ...
    for (let p = 0; p < defaultData.length; p++) {
        let performers = defaultData[p].performers
        if (performers) {
            for (let t = 0; t < performers.length; t++) {
                var performer = performers[t]
                if (stringNotAlreadyPresent(performer, defaultPerformers)) defaultPerformers.push(performer)
            }
        }
    }
    ...
    if (defaultTags) {
        for (let i = defaultTags.length - 1; i >= 0; i--) {
            let tagId = getTagId(defaultTags[i])
    ...
    if (defaultPerformers) {
        for (let i = defaultPerformers.length - 1; i >= 0; i--) {
            let tagId = getPerformerId(defaultPerformers[i])
    ...
    for (let o = 0; o < defaultTags.length; o++) {
        log.Info("[DefaultDataForPath]: Added tag " + defaultTags[o] + " to " + name + " " + ID)
    }
    for (let o = 0; o < defaultPerformers.length; o++) {
