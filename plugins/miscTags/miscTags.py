...
def processScene(scene):
    ...
    if tags_cache[skip_tag] not in [x['id'] for x in scene['tags']]:
        ...
        if len(tags) > 0:
            log.debug('processing scene %s, checking if tags need to be added: %s' % (scene['title'],tags,))
            ...
            for t in tags:
                ...
                if tags_cache[t] not in new_scene['tag_ids']:
                    ...
                    update=True
            if update:
                log.info('Adding tags to scene: %s, tags: %s' % (scene['title'], tags,))
...
def processFlatStudio(scene,tags):
    log.debug(scene)
    if scene['studio']:
        if scene['studio']['id'] in [x.strip() for x in settings['flatStudio'].split(',')]:
...
def processScenes():
    ...
    if skip_tag not in tags_cache:
        tags_cache[skip_tag]=stash.find_tag(skip_tag, create=True).get("id")
    count=stash.find_scenes(f={"tags":{"depth":0,"excludes":[tags_cache[skip_tag]],"modifier":"INCLUDES_ALL","value":[]}},filter={"per_page": 1},get_count=True)[0]
    log.info(str(count)+' scenes to process.')
    i=0
    for r in range(1,int(count/per_page)+2):
        log.info('adding tags to scenes: %s - %s %0.1f%%' % ((r - 1) * per_page,r * per_page,(i/count)*100,))
        scenes=stash.find_scenes(f={"tags":{"depth":0,"excludes":[tags_cache[skip_tag]],"modifier":"INCLUDES_ALL","value":[]}},filter={"page":r,"per_page": per_page})
        for s in scenes:
            processScene(s)
            i=i+1
