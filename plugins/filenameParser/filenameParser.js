...
function matchNames(parts, name, aliases) {
    ...
    for (let i = 0; i < names.length; i++) {
        name = names[i].replace(cleanRegex, "").toLowerCase()
...
function cleanFilename(name) {
    ...
    var blockList = ["mp4", "mov", "zip", "xxx", "4k", "4096x2160", "3840x2160", "2160p", "1080p", "1920x1080", "60fps", "30fps", "repack", "ktr"]
...
function matchDate(parts, result) {
    if (parts.length < 3 || !/^(\d{2}|\d{4})$/.test(parts[0]) || !/^\d{2}$/.test(parts[1]) || !/^\d{2}$/.test(parts[2])) {
        return 0
    }
