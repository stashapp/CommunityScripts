...
| 3.0.2 | Review polish: star buckets round to nearest (6.1/10 was showing 4★ via `ceil`); decimal settings (`fantasyConsensus`, `qualityZThreshold`) switched to STRING type because Stash's NUMBER input is integer-only; every setting description now states its default; chart canvases get a real `min-height` so Chart.js hover/tooltips register; long names ellipsize/wrap instead of clipping outside cards and tables. |
