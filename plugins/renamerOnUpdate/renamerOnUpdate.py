...
def extract_info(scene: dict, template: None):
    ...
    if scene.get("title"):
        ...
        scene_information["title"] = re.sub(rf"{scene_information['file_extension']}$", "", scene["title"])
        if PREPOSITIONS_REMOVAL:
            for word in PREPOSITIONS_LIST:
                scene_information["title"] = re.sub(rf"^{word}[\s_-]", "", scene_information["title"])
        ...
        for _ in range(1, len(scene_info["new_directory"].split(os.sep))):
            ...
            if parent_id:
                ...
                cursor.execute(
                    ...
                    [new_id, scene_info["new_directory"], parent_id[0][0], mod_time, mod_time, mod_time, None],
                )
                stash_db.commit()
                folder_id = new_id
                break
