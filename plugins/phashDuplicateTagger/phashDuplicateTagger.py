...
def tag_files(group):
    ...
    if not keep_scene:
        ...
        if config.UNKNOWN_TAG_NAME:
            ...
            for scene in group:
                ...
                stash.update_scenes(
                    {"ids": [scene.id], "title": f"[PDT: {total_size}|{group_id}U] {scene.title}", "tag_ids": {"mode": "ADD", "ids": tag_ids}}
                )
        return
    ...
    for scene in group:
        if scene.id == keep_scene.id:
            ...
            stash.update_scenes(
                {"ids": [scene.id], "title": f"[PDT: {total_size}|{keep_scene.id}K] {scene.title}", "tag_ids": {"mode": "ADD", "ids": tag_ids}}
            )
        else:
            tag_ids = []
            ...
            stash.update_scenes(
                {"ids": [scene.id], "title": f"[PDT: {total_size}|{keep_scene.id}R] {scene.title}", "tag_ids": {"mode": "ADD", "ids": tag_ids}}
            )


def clean_scenes():
    scene_count, scenes = stash.find_scenes(
        f={"title": {"modifier": "MATCHES_REGEX", "value": "^\\[PDT: .+?\\]"}}, fragment="id title", get_count=True
    )
    ...
    # Remove Tags
    for tag in get_managed_tags():
        scene_count, scenes = stash.find_scenes(f={"tags": {"value": [tag["id"]], "modifier": "INCLUDES", "depth": 0}}, fragment="id", get_count=True)
        if not scene_count > 0:
            continue
        log.info(f'removing tag {tag["name"]} from {scene_count} scenes')
        stash.update_scenes({"ids": [s["id"] for s in scenes], "tag_ids": {"mode": "REMOVE", "ids": [tag["id"]]}})


def get_managed_tags(fragment="id name"):
    tags = stash.find_tags(f={"name": {"value": "^\\[Reason", "modifier": "MATCHES_REGEX"}}, fragment=fragment)
    tag_name_list = [
