                    ...
                    for g in md["galleries"]:
                        for f in g["files"]:
                            res = stash.find_galleries(
                                f={
                                    "checksum": {"value": f["md5"], "modifier": "EQUALS"},
                                    "tags": {"depth": 0, "excludes": [skip_sync_tag_id], "modifier": "INCLUDES_ALL", "value": []},
                                }
                            )
                            for gal in res:
            ...
            if settings["createMovieFromScene"]:
                if "movies" in md:
                    ...
                    for m in md["movies"]:
                        ...
                        movies = []
                        for u in m["urls"]:
                            sm = stash.find_movies(f={"url": {"modifier": "EQUALS", "value": u["url"]}})
                            log.debug("sm: %s" % (sm,))
                            movies.extend(sm)
                        if len(movies) == 0:
                            for u in m["urls"]:
                                ...
                                if movie_scrape["studio"]:
                                    new_movie["studio_id"] = movie_scrape["studio"]["stored_id"]
                                log.debug("new movie: %s" % (new_movie,))
                                nm = stash.create_movie(new_movie)
                                movies.append(nm)
                        movies_to_add.extend([{"movie_id": x["id"], "scene_index": m["scene_index"]} for x in movies])
                    if len(movies_to_add) > 0:
                        new_scene["movies"] = []
                        for m in movies_to_add:
                            if m["movie_id"] not in [x["movie"]["id"] for x in s["movies"]]:
...
def processAll():
    ...
    count = stash.find_scenes(
        f={
            "stash_id_endpoint": {"endpoint": "", "modifier": "NOT_NULL", "stash_id": ""},
            "has_markers": "false",
            "tags": {"depth": 0, "excludes": [skip_sync_tag_id], "modifier": "INCLUDES_ALL", "value": []},
        },
        filter={"per_page": 1},
    ...
    for r in range(1, int(count / per_page) + 1):
        ...
        scenes = stash.find_scenes(
            f={"stash_id_endpoint": {"endpoint": "", "modifier": "NOT_NULL", "stash_id": ""}, "has_markers": "false"},
...
def submitGallery():
    ...
    for r in range(1, math.ceil(count / per_page) + 1):
        ...
        galleries = stash.find_galleries(
            ...
            filter={"page": r, "per_page": per_page, "sort": "created_at", "direction": "DESC"},
            fragment=scene_fgmt,
        )
        for g in galleries:
                            ...
                            for p in g["performers"]:
                                ...
                                for sid in p["stash_ids"]:
                                    performers = stash.find_performers(
                                        f={"stash_id_endpoint": {"endpoint": sid["endpoint"], "stash_id": sid["stash_id"], "modifier": "EQUALS"}}
                                    )
                                    if len(performers) > 0:
...
if "mode" in json_input["args"]:
    ...
    if "submitScene" in PLUGIN_ARGS:
        skip_submit_tag_id = stash.find_tag("[Timestamp: Skip Submit]", create=True).get("id")
        query = {"has_markers": "true", "tags": {"depth": 0, "excludes": [skip_submit_tag_id], "modifier": "INCLUDES_ALL", "value": []}}
        submitScene(query)
