...
def processPerformers():
    performers = stash.find_performers(
        f={"tags": {"depth": 0, "excludes": [], "modifier": "INCLUDES_ALL", "value": [tag_stashbox_performer_gallery]}}
    )
...
def setPerformerPicture(img):
    if len(img["performers"]) == 1:
        ...
        if res.status_code == 200:
            ...
            new_performer = {"id": img["performers"][0]["id"], "image": "data:{0};base64,{1}".format(res.headers["Content-Type"], encoded)}
...
def processQueue():
    ...
    if stash.get_configuration()["plugins"]["stashdb-performer-gallery"] == settings["queue"]:
        ...
        stash.configure_plugin(
            ...
            {"queue": stash.get_configuration()["plugins"]["stashdb-performer-gallery"]["queue"].removeprefix(settings["queue"])},
        )
