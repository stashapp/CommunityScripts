...
def processPerformers():
    performers=stash.find_performers(f={"tags": {"depth": 0, "excludes": [], "modifier": "INCLUDES_ALL", "value": [ tag_stashbox_performer_gallery ]}})
...
#        return stash_boxes[endpoint]
    for sbx_config in stash.get_configuration()['general']['stashBoxes']:
        if sbx_config['endpoint']==endpoint:
            stashbox = StashBoxInterface({'endpoint': sbx_config['endpoint'], 'api_key': sbx_config['api_key']})
            stash_boxes[endpoint] = stashbox
            return stashbox

def processPerformerStashid(endpoint,stashid,p):
    log.info('processing performer %s, %s  endpoint: %s,  stash id: %s' % (p['name'],p['id'],endpoint,stashid,))
    ...
    if stashbox:
        ...
        if endpoint not in index['galleries']:
            gallery_input={
                'title': '%s - %s ' % (p['name'], endpoint[8:-8],),
                'urls':['%s/performers/%s' % (endpoint[:-8],stashid,)],
        ...
        for img in perf['images']:
            ...
            if not os.path.exists(filename):
                log.info('Downloading image %s to %s' % (img['url'], filename,))
    ...
    if len(img['performers'])==1:
        ...
        if res.status_code==200:
            ...
            new_performer ={
                ...
                'image':'data:{0};base64,{1}'.format(res.headers['Content-Type'], encoded)
            }
...
def processQueue():
    ...
    if stash.get_configuration()['plugins']['stashdb-performer-gallery']==settings['queue']:
        ...
        stash.configure_plugin('stashdb-performer-gallery',{'queue':stash.get_configuration()['plugins']['stashdb-performer-gallery']['queue'].removeprefix(settings['queue'])})
        stash.run_plugin_task('stashdb-performer-gallery','Process Performers')

def relink_images():
    images=stash.find_images(f= {"path": {"modifier": "INCLUDES","value": settings['path']},"performer_count": {"modifier": "EQUALS","value": 0}})
    log.debug(images)
    for img in images:
        processImages(img)
...
tag_stashbox_performer_gallery = stash.find_tag('[Stashbox Performer Gallery]', create=True).get("id")
...
if 'mode' in json_input['args']:
    ...
    if 'performer' in json_input['args']:
        ...
        if tag_stashbox_performer_gallery in [x['id'] for x in p['tags']]:
            processPerformer(p)
        stash.metadata_scan(paths=[settings['path']])
        stash.run_plugin_task('stashdb-performer-gallery','relink missing images',args={})
    ...
    if json_input['args']['hookContext']['type']=='Performer.Update.Post':
        stash.run_plugin_task('stashdb-performer-gallery','Process Performers',args={'performer':id})
