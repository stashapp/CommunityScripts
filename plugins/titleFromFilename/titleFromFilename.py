...
if api_version > API_VERSION_BF_FILES:  # only needed for versions after files refactor
    ...
    graphql.exit_plugin(f"Stash with API version:{api_version} is not supported. You need at least {API_VERSION_BF_FILES}")
...
while i >= 0:
    ...
    if updated_scene:
        graphql.exit_plugin(f"Scene title updated after {MAX_RETRY_COUNT - i} tries. Title:{updated_scene.get('title')}")
